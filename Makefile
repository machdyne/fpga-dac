...
audio_lakritz:
	mkdir -p output
	yosys -DECP5 -q -p "synth_ecp5 -top audio -json output/audio.json" $(RTL)
	nextpnr-ecp5 --12k --package CABGA256 --lpf boards/lakritz_v0.lpf --json output/audio.json --textcfg output/audio.config
	ecppack -v --compress --freq 2.4 output/audio.config --bit output/audio.bit

audio_noir:
	mkdir -p output
	yosys -DECP5 -q -p "synth_ecp5 -top audio -json output/audio.json" $(RTL)
	nextpnr-ecp5 --45k --package CABGA256 --lpf boards/noir_v0.lpf --json output/audio.json --textcfg output/audio.config
	ecppack -v --compress --freq 2.4 output/audio.config --bit output/audio.bit
